...
def pre_process_image(img):
    ...
    kernel_Line = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 20))
    kernel_vert = cv2.getStructuringElement(cv2.MORPH_RECT, (20, 1))
    proc = cv2.dilate(proc, kernel)
    cv2.imwrite('test.jpg', proc)
    return proc
...
def gridparse():
    ...
    while (1):
        ...
        cv2.imwrite('masked.jpg', processed)
        ...
        height_start = [int(size * 400 / 4219), int(size * 580 / 4219)]
        width_start = [int(size * 470 / 4219), int(size * 380 / 4219), int(size * 570 / 4219)]
        ...
        for y in x:
            ...
            for i in y:
                valx = i * change
                color1 = unique_count_app(cropped[height_start[0] + valx: height_start[0] + valx + thick,
                                       width_start[0] + valy:width_start[0] + valy + thick])
                color2 = unique_count_app(cropped[height_start[1] + valx: height_start[1] + valx + thick,
                                      width_start[1] + valy:width_start[1] + valy + thick])
                color3 = unique_count_app(cropped[height_start[1] + valx: height_start[1] + valx + thick,
                ...
                cv2.imwrite(str(color1) + str(chr(ord('A') + col)) + str(cell) + "#1.png", cropped[height_start[0] + valx: height_start[0] + valx + thick,
                                          width_start[0] + valy:width_start[0] + valy + thick])
                cv2.imwrite(str(color2) + str(chr(ord('A') + col)) + str(cell) + "#2.png", cropped[height_start[1] + valx: height_start[1] + valx + thick,
                                          width_start[1] + valy:width_start[1] + valy + thick])
                cv2.imwrite(str(color3) + str(chr(ord('A') + col)) + str(cell) + "#3.png", cropped[height_start[1] + valx: height_start[1] + valx + thick,
                                          width_start[2] + valy:width_start[2] + valy + thick])
                time.sleep(1)
                request.update(dic)
                cell += 1
            col += 1
        print(json.dumps(request, sort_keys=True, indent=4)) #uncomment this line to show the json
        time.sleep(5)
        break
